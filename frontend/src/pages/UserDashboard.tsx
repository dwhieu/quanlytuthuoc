  ...
  useEffect(() => {
    ...
    const fetchProfile = async () => {
      ...
      try {
        ...
        const res = await fetch(`${API_BASE}/auth/user/${encodeURIComponent(username)}`, { headers });
        if (!res.ok) {
  ...
  useEffect(() => {
    ...
    const loadCatalog = async () => {
      ...
      try {
        ...
        const res = await fetch(`${API_BASE}/drugs`, { headers, signal: controller.signal });
        if (!res.ok) {
          const message = await res.text();
                    ...
                    <div className="d-flex gap-2">
                      <button className="btn btn-primary" disabled={passwordSaving} onClick={async () => {
                        ...
                        try {
                          ...
                          const res = await fetch(`${API_BASE}/auth/user/${encodeURIComponent(username || '')}/password`, {
                            method: 'PUT',
                            headers,
                ...
                <div className="d-flex gap-2 mt-4">
                  <button className="btn btn-primary" disabled={saving} onClick={async () => {
                    ...
                    try {
                      ...
                      const res = await fetch(`${API_BASE}/auth/user/${encodeURIComponent(username || '')}`, {
                        method: 'PUT',
                        headers,
